...
public class QuestionboxBowser : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.CompareTag("Mario_Top"))
        {
            ...
            if (IsBump == false)
            {
                this.GetComponent<AudioSource>().PlayOneShot(BumpClip);
                IsBump = true;
                stopTime = false;
            }
    ...
    void SpawnPowerup()
    {
        ...
        this.GetComponent<AudioSource>().PlayOneShot(BowserClip);
    ...
    public void Reset()
    {
        if (IsActive == false)
        {
            ...
            PowerupGameObject.GetComponent<MagicBowserPowerup>().Reset();
        }
